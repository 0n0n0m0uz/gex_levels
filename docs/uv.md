...
you should think about your intended audience before you begin the project because there are
different approaches to packaging depending on who and how it will be used.

simple scripts that only use the standard internal python libraries can be emailed s a single .py
...
virtualenv is stricty python packages using pip, no other infrastructure or ecosystem packages 
...
more local centric where the environment is inside a single package only whereas conda is like
a sandbox w more sprawl for a category of projects

uv doesnt require activation of env before running script either and is locked down to exact
